...
class AdminOrderController extends Controller
{
    ...
    function list(Request $request)
    {
    	...
    	// $orders=Order::where('name','LIKE', "%{$keyword}%")->paginate(3);
        ...
        $status = $request->input('status');
         ...
         if($status){
             if($status == 'processing'){
                 ...
                 $orders = Order::where('pay_method', 1)->paginate(10);
             }elseif($status == 'store'){
                 $orders = Order::where('pay_method', 2)->paginate(10);
             }else{
                 $list_action = [
                     'restore' => 'Khôi phục',
              ...
              $orders = Order::where('name', 'LIKE', "%{$keyword}%")->orwhere('price', 'LIKE', "%{$keyword}%")->orwhere('qty', $keyword)->paginate(10);
             }
    ...
    public function detail($id){
        ...
        return view('admin.order.detail', compact('order', 'products', 'qty'));
    }
}
