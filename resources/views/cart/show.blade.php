    ...
    <div id="wrapper" class="wp-inner clearfix">
                    ...
                    <tbody>
                       @foreach (Cart::content() as $cart)
                        <tr>
                            ...
                            <td>
                                <a href="" title="" class="name-product">{{Str::of($cart->name)->limit(50) }}</a>
                            </td>
                            <td>{{ number_format($cart->price, 0, '', '.') }}đ</td>
                            <td>
                                <input type="number" min="1" data-rowId="{{ $cart->rowId }}" max="{{ $cart->options->max_qty }}" name="qty[{{ $cart->rowId }}]" value="{{ $cart->qty }}" class="num-order">
                            </td>
                            <td id="sub-total-{{$cart->rowId}}" >{{ number_format($cart->subtotal, 0, '', '.') }}đ</td>
                            <td>
                                <a href="{{ url('remove', $cart->rowId) }}" title="" class="del-product"><i class="fa fa-trash-o"></i></a>
                            </td>
                        </tr>
                        @endforeach
        ...
        <div class="section" id="action-cart-wp">
            <div class="section-detail">
                ...
                <a href="{{ url('destroy') }}" title="" id="delete-cart">Xóa giỏ hàng</a>
            </div>
        </div>
        @else
            ...
            $(".num-order").change(function() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $("#sub-total1-" + rowId).text(data.sub_total);
                        $("#total-price span").text(data.total);
                        $("#total-price1").text(data.sub_total);
